...
function NewPassword() {
  ...
  const validateForm = () => {
    ...
    if (!formData.confirmPassword) {
      newErrors.confirmPassword = getTranslation("validation.confirmPasswordRequired", currentLanguage);
        ...
        if (typeof serverErrors === "object") {
          if (serverErrors.detail) {
            ...
            Object.keys(serverErrors).forEach((key) => {
              processedErrors[key] = Array.isArray(serverErrors[key]) ? serverErrors[key][0] : serverErrors[key];
            });
  ...
  return (
    <Container fluid className="d-flex justify-content-center align-items-center" style={{ height: "100vh", background: "#D4E2F6" }}>
      <Row className="w-100 justify-content-center align-items-center">
        <Col xs={12} lg={6} className="d-none d-lg-flex justify-content-center">
          <img src={avatar} alt="Password Reset Illustration" className="img-fluid mb-4" style={{ maxWidth: "300px", height: "auto" }} />
        </Col>
        <Col xs={12} md={10} lg={6} xl={5}>
          <Card className="shadow-sm" style={{ width: "100%", maxWidth: "550px", borderRadius: "1.5rem", padding: "2rem" }}>
            <Row className="mb-4">
              <Col className="text-center">
                <h3 className="mb-3">{getTranslation("auth.newPassword", currentLanguage)}</h3>
                <p className="text-muted">{getTranslation("auth.newPasswordDesc", currentLanguage)}</p>
              </Col>
            </Row>
                ...
                <Col>
                  <Form.Group controlId="formPassword">
                    <InputGroup>
                      ...
                      />
                      <InputGroup.Text onClick={() => setShowPassword((p) => !p)} style={{ cursor: "pointer" }}>
                ...
                <Col>
                  <Form.Group controlId="formConfirmPassword">
                    <InputGroup>
                      ...
                      />
                      <InputGroup.Text onClick={() => setShowConfirmPassword((p) => !p)} style={{ cursor: "pointer" }}>
              ...
              <Row className="mb-3">
                <Col className="text-center">
                  <Button type="submit" className="w-100 py-2" disabled={loading || !token}>
                    {loading ? (
                      <><span className="spinner-border spinner-border-sm me-2" aria-hidden="true"></span>{getTranslation("auth.resetting", currentLanguage)}</>
            ...
            <Row>
              <Col className="text-center">
                <Link to="/login" className="text-decoration-none">{getTranslation("auth.backToLogin", currentLanguage)}</Link>
              </Col>
            </Row>
          </Card>
        </Col>
